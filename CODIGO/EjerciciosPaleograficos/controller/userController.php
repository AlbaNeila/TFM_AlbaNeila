<?php
    ...
    switch($method){
        ...
        case 'deleteStudent':
            deleteStudent();
            break;
        case 'checkUpdateGridStudent':
            checkUpdateGridStudent();
            break;
    }
    ...
    function deleteStudent(){
        $idStudent = mysqli_real_escape_string($GLOBALS['link'],$_POST['idStudent']);
        
       $result = mysqli_query($GLOBALS['link'],"DELETE FROM usuario WHERE usuario.idUsuario= '".$idStudent."'");
    ...
    function checkUpdateGridStudent(){
        $row = $_POST["row"];
        $row = json_decode("$row",true);
        $idStudent = $_POST['idStudent'];
        
        $result = mysqli_query($GLOBALS['link'],"SELECT usuario.nombre FROM usuario WHERE usuario.usuario= '".$row[4]."' and usuario.idUsuario<>'".$idStudent."'");
        ...
        if($result!=FALSE){
            if(!$fila=mysqli_fetch_assoc($result)) { //Si no hay filas es que no existe otro usuario con el mismo dni, por lo que actualizamos la fila
                $result2 = mysqli_query($GLOBALS['link'],"UPDATE usuario SET usuario.nombre='".utf8_decode($row[1])."', usuario.apellidos='".utf8_decode($row[2])."',usuario.email='".utf8_decode($row[3])."' ,usuario.usuario='".utf8_decode($row[4])."',usuario.password='".md5($row[5])."' WHERE usuario.idUsuario='".$idStudent."'");
                if($result2!=FALSE)
                    echo 1;
            }
            else{
